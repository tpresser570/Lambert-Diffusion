...
def get_model(config):
    ...
    elif config.data.dataset == 'unconditional_lambert':
        return NCSNv2(config).to(config.device)
...
class NCSNRunner():
    ...
    def train(self):
        ...
        self.config.input_dim = self.config.data.image_size * self.config.data.image_size_feats * self.config.data.channels
                ...
                if step % self.config.training.snapshot_freq == 0:
                    ...
                    if self.config.training.snapshot_sampling:
                        ...
                        init_samples = torch.rand(36, self.config.data.channels,
                                                  self.config.data.image_size_feats, self.config.data.image_size,
                        ...
                        sample = all_samples[-1].view(all_samples[-1].shape[0], self.config.data.channels,
                                                      self.config.data.image_size_feats,
        ...
        if not self.config.sampling.fid:
            if self.config.sampling.inpainting:
                ...
                init_samples = torch.rand(width, width, self.config.data.channels,
                                          ...
                                          device=self.config.device)
                init_samples = data_transform(self.config, init_samples)
                #TODO fix inpainting dimensions 
